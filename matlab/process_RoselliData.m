%% Calculating surface area and volume for cells of various geometry
%% Alexey Ryabov 2020
% the script loads data, average linear dimensions for each species and location 
% find area and volume of cells
...
%shape IDs are in CalculationsOfCellVolume&Surface.pdf and also 

clearvars;

tLinDim = readtable('..\Roselli\ALL linear dimensions.xlsx');
...
%remove spaces
tLinDim.CodeEcosystem = upper(erase(tLinDim.CodeEcosystem, ' '));

%get the lenght of ecosystem code
CodeEcosystem_len = rowfun(@strlength, tLinDim(:, 'CodeEcosystem'));
%substract sample letter
tLinDim.Sample = extractAfter(tLinDim.CodeEcosystem, CodeEcosystem_len.Var1 - 1);
%remove sample letter from the ecosystem code
tLinDim.CodeEcosystem =  extractBefore(tLinDim.CodeEcosystem, CodeEcosystem_len.Var1);


%% find average linear dimensions for each species
SpecAndSampleAverage = 1;  % if 1 then avarage for species and sample, otherwise only for species
%NonNaNs_idx =  ~(isnan(tLinDim.Area) | isnan(tLinDim.Volume));
tLinDim.Genus_species = strtrim(tLinDim.Genus_species);

%store the order of rows
tLinDim.ID = (1:size(tLinDim, 1))';
tLinDim.SpeciesName =  tLinDim.Genus_species;
tLinDim =  CleanSpeciesNames(tLinDim);
tLinDim.Genus_species = tLinDim.SpeciesName;
tLinDim.SpeciesName = [];

%%!!update field c with Pleorosigma
tLinDim(strcmp(tLinDim.Genus_species_view_, 'Pleurosigma angulatum  (valvar view)'), {'c'}) = tLinDim(strcmp(tLinDim.Genus_species_view_, 'Pleurosigma angulatum  (valvar view)'), {'b'});
tLinDim(strcmp(tLinDim.Genus_species_view_, 'Pleurosigma sp. 1  (valvar view)'), {'c'}) = tLinDim(strcmp(tLinDim.Genus_species_view_, 'Pleurosigma sp. 1  (valvar view)'), {'b'});
tLinDim(strcmp(tLinDim.Genus_species_view_, 'Pleurosigma spp.  (valvar view)'), {'c'}) = tLinDim(strcmp(tLinDim.Genus_species_view_, 'Pleurosigma spp.  (valvar view)'), {'b'});
...
tLinDim.Area       = tLinDim.Volume;
tLinDim.Sphericity = tLinDim.Volume;
tLinDim.Eqv_Rad_v   = tLinDim.Volume;
tLinDim.Dim1   = tLinDim.Volume;
tLinDim.Dim2   = tLinDim.Volume;
tLinDim.Dim3   = tLinDim.Volume;
...
tLinDim.DimMin = DimsSorted(:, 1);
tLinDim.DimMid = DimsSorted(:, 2);
tLinDim.DimMax = DimsSorted(:, 3);
tLinDim.DmMid2Min = tLinDim.DimMid./tLinDim.DimMin;
tLinDim.DmMax2Mid = tLinDim.DimMax./tLinDim.DimMid;

tLinDim.Eqv_Rad_v = (tLinDim.Volume/(4/3 * pi)).^(1/3);
tLinDim.Sphericity = tLinDim.Area./(4 * pi * tLinDim.Eqv_Rad_v.^2);
...
%save data
writenewtable(tLinDim,'..\Roselli\ALL linear dimensions_VA.xlsx')
